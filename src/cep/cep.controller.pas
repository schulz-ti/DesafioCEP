...
begin
  if Length(Trim(FCep)) < 9 then
    raise Exception.Create('CEP inv·lido para pesquisa.');
end;

procedure TCEPController.ValidarEnderecoCompleto;
begin
  if Length(Trim(FUF)) < 2 then
    raise Exception.Create('UF inv·lido para pesquisa.')

  else if Length(Trim(FLocalidade)) < 3 then
    raise Exception.Create('Localidade inv·lida para pesquisa.')

  else if Length(Trim(FLogradouro)) < 3 then
    raise Exception.Create('Logradouro inv·lido para pesquisa.');
end;
